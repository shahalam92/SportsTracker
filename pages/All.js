...
import {View,Text,FlatList,TouchableHighlight,TouchableOpacity,BackHandler,Dimensions,Alert,ScrollView,
  RefreshControl,SafeAreaView,NetInfo} from 'react-native';
...
export default class All extends Component{
    ...
    constructor(props){
        ...
        this.state={text:'',Data:this.Data}
    }
    ...
    handleSearch = () => {
      ...
      gotoEventDetails=(item,name,age,gender)=>{
        ...
        if(item.participants.length==0){
         ...
         this.props.navigation.navigate('EventDetails',{events:item,name:name,gender:gender,age:age,token:this.token})
      }
    ...
    render(){
        ...
        fontSize:20,color:'#ffffff'}}>All events</Text>
                    </View>
               {/* </View> */}
                </View>
          ...
          renderItem={({ item,index }) =>
            (
              <View style={{ flexDirection: 'row', borderBottomColor: '#E6E6E6', borderWidth: 0.5, backgroundColor: index%2==0?"#F4F4F8":'#ffffff'  ,paddingBottom:20,paddingTop:20}}>
                <TouchableOpacity onPress={()=>this.gotoEventDetails(item,item.name,item.age,item.gender)}>
